...
namespace Gamekit3D
{
    ...
    public class TargetScanner
    {
        ...
        public PlayerControl Detect(Transform detector, bool useHeightDifference = true)
        {
            //if either the player is not spwned or they are spawning, we do not target them
            if (PlayerControl.instance == null /*|| PlayerControl.instance.respawning*/)
                return null;

            Vector3 eyePos = detector.position + Vector3.up * heightOffset;
            Vector3 toPlayer = PlayerControl.instance.transform.position - eyePos;
            Vector3 toPlayerTop = PlayerControl.instance.transform.position + Vector3.up * 1.5f - eyePos;
